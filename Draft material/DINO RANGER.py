...
class Config:
    ...
    RESUME_CHECKPOINT_DIR = "/kaggle/input/309-dino-ranger/training_outputs_novel/checkpoints"
    RESUME_EPOCH = 408
...
def validate_resume_config(config):
    if not config.RESUME_TRAINING: return 1, None
    ...
    if config.RESUME_EPOCH is not None:
        ...
        if os.path.exists(path): return config.RESUME_EPOCH + 1, path
        else: print(f"ERROR: No checkpoint for epoch {config.RESUME_EPOCH}"); return 1, None
    else:
        files = glob.glob(os.path.join(checkpoint_dir, "checkpoint_epoch_*.pth"))
        if not files: return 1, None
