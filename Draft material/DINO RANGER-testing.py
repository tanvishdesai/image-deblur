...
# =================================================================================================
# SECTION 2: MODEL ARCHITECTURE (Copied from proposed_solution_2.py)
# =================================================================================================
...
class TestConfig:
    ...
    CHECKPOINT_PATH = "/kaggle/input/309-dino-ranger/training_outputs_novel/checkpoints/checkpoint_epoch_508.pth" # <<< UPDATE THIS PATH
    DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
    # SCALE_FACTOR is now determined dynamically from the dataset folders.

    # --- HybridSwinNet Parameters (must match the trained model from proposed_solution_2.py) ---
    HYBRID_NUM_FEAT = 64
    HYBRID_NUM_BLOCK = 16
    HYBRID_NUM_HEAD = 8
        ...
        for lr_folder in sorted(scale_folders):
            ...
            if top_samples:
                ...
                for i, sample in enumerate(top_samples[:5]):
                    ...
                    save_path = os.path.join(output_dir, f"{name}_x{scale}_{sample['name']}_rank{i+1}_psnr_{psnr_val:.2f}_dino-ranger.png")
