...
# --- Additional imports for Novel Degradation Pipeline ---
import cv2
...
class Config:
    ...
    SCALE_FACTOR = 4
    # LR_FOLDER no longer needed - we generate LR images with NovelDegradationPipeline
    HR_FOLDER = "DF2K_train_HR"

    # --- Resume Training Options ---
    RESUME_TRAINING = True
    RESUME_CHECKPOINT_DIR = "/kaggle/input/dino-ranger-w-degrade"
    RESUME_EPOCH = 410
    RESUME_CHECKPOINT_PATH = "/kaggle/input/dino-ranger-w-degrade/training_outputs_novel/checkpoints/checkpoint_epoch_410.pth"  # Direct path to a specific checkpoint file
...
# =================================================================================================
# SECTION 2.5: <<< NOVEL DEGRADATION PIPELINE >>> REALISTIC DEGRADATION MODEL
# =================================================================================================
...
print("   - Realistic degradations: Chromatic aberration, sensor noise, motion blur")
print("   - JPEG compression, lens distortion, random resizing")
print("   - This replaces simple bicubic downsampling for robustness")
print("-" * 80)


# =================================================================================================
# SECTION 3: DATASET AND DATALOADER (MODIFIED for Novel Degradation Pipeline)
# =================================================================================================
class DF2KDataset(Dataset):
    def __init__(self, config):
        self.config = config
        self.hr_path = os.path.join(config.DATASET_PATH, config.HR_FOLDER)
        # No longer need LR path - we generate LR images on-the-fly
        self.hr_image_files = sorted(glob.glob(os.path.join(self.hr_path, "*.png")))
        self.to_tensor = T.ToTensor()
        # Initialize the novel degradation pipeline
    ...
    def __getitem__(self, index):
        ...
        # Get random HR patch first
        hr_patch = self.get_random_hr_patch(hr_image)
        
        # Apply degradation pipeline to generate realistic LR patch
        lr_patch = self.degradation_pipeline.degrade_image(hr_patch)
        
        # Apply data augmentation to both patches together
    ...
    def get_random_hr_patch(self, hr_img):
        """Extract a random HR patch from the high-resolution image."""
        ...
        if hr_w < hr_patch_size or hr_h < hr_patch_size:
            # If image is too small, resize it to at least patch size
            scale = max(hr_patch_size / hr_w, hr_patch_size / hr_h) * 1.1
            new_w, new_h = int(hr_w * scale), int(hr_h * scale)
            hr_img = hr_img.resize((new_w, new_h), Image.BICUBIC)
            hr_w, hr_h = new_w, new_h
...
print("   - MAJOR IMPROVEMENT: Using NovelDegradationPipeline instead of simple bicubic")
print("   - This makes the model robust to real-world image corruptions")
print("   - No longer requires pre-generated LR images")
print("-" * 80)
...
if __name__ == '__main__':
    ...
    if not os.path.exists(config.DATASET_PATH) or not os.path.exists(os.path.join(config.DATASET_PATH, config.HR_FOLDER)):
        print("\n" + "!"*80 + "\n! ERROR: HR dataset path not found. Please check `DATASET_PATH` and `HR_FOLDER` in Config.")
        print("! NOTE: We no longer need LR images - they are generated on-the-fly with NovelDegradationPipeline.\n" + "!"*80)
